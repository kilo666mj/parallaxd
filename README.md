...
# Edit inventory, then put secrets in the gitignored group_vars/all.yml.
ansible-playbook playbook.yml
```

Start with the documentation-safe checks in
[`ansible/group_vars/parallaxd.yml`](ansible/group_vars/parallaxd.yml), then
replace the `example.com` targets and providers with your own. The play creates
service users, generates signing keys on their owning hosts, validates rendered
configuration, installs systemd units, and applies source-restricted firewall
rules. See [Deploying](#deploying) for HA, private probers, and secret handling.

The binaries require Linux; deployment additionally requires Ansible and the
`ansible.posix` collection. Development requires the Go version declared in
[`go.mod`](go.mod).
...
```json
{
  ...
  "tls_expiry_warning": "360h"
}
```

The warning is transition-based like every other failure: it alerts once,
remains visible while the certificate is inside the warning window, and
recovers after a renewed certificate is observed. Leaving the field empty
still validates the certificate and reports an expired certificate as down,
but does not warn proactively.
...
"notification_routes": [
  ...
  {"name":"page public sites", "destination":"pager",
   "checks":["website","website-tls"], "kinds":["down"]}
],
"escalations": [
  {"name":"unacknowledged public outage", "destination":"pager",
   "after":"10m", "checks":["website","website-tls"],
   "kinds":["down"]}
]
```
...
3. Promote it with an authenticated request that explicitly records the
   ...
   curl -X POST https://standby.internal:8972/v1/ha/promote \
     -H @/run/secrets/parallaxd-operator-header \
     -H 'Content-Type: application/json' \
